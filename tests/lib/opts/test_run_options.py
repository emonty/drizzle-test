...
def handle_user_opts(variables, defaults):
    ...
    # dtr mode is a special case and we have different suites
    # paths, etc.  We change our default list of suites
    # if we are using dtr mode and the user hasn't specified 
    # any suites (they want default dtr suites)
    if variables['mode'] == 'dtr':
        if variables['suitelist'] == defaults['suitelist']:
            variables['suitelist'] = defaults['dtrsuitelist']
    return variables
