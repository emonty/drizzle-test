...
"""Defines the adapter for a simple Drizzle client"""
...
class DrizzleClient:
  ...
  def execute(self, statement):
    ...
    client_cmd= "%s -uroot --port=%d -e\"%s\"" % (os.path.join(self._basedir, "client", "drizzle")
                                        , self._port
                                        , statement)
  ...
  def executeFile(self, file):
    ...
    client_cmd= "%s -uroot --port=%d < %s" % (os.path.join(self._basedir, "client", "drizzle")
                                        , self._port
                                        , file)
    (retcode, output)= commands.getstatusoutput(client_cmd)
    if retcode != 0:
