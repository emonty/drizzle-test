...
"""Various reports on sqlbench command results"""
...
, IF (AVG(i.seconds) >= agg.avg_seconds
  , CONCAT('-', ROUND(((AVG(i.seconds) - agg.avg_seconds) / agg.avg_seconds) * 100, 2), '%%')
  , CONCAT('+', ROUND(((agg.avg_seconds- AVG(i.seconds)) / agg.avg_seconds) * 100, 2), '%%')
  ) as pct_diff_from_avg
, ROUND(agg.avg_seconds - (AVG(i.seconds)), 2) AS diff_from_avg
, ROUND(agg.min_seconds,2) AS min_seconds
, ROUND(agg.max_seconds,2) AS max_seconds
, ROUND(agg.avg_seconds,2) AS avg_seconds
, FORMAT(ROUND(agg.stddev_seconds,2),2) AS stddev_seconds
FROM bench_config c
...
INNER JOIN (
  ...
  FROM sqlbench_run_iterations iter
  WHERE run_id IN (%s)
  GROUP BY engine, operation_name
) AS agg
  ON i.operation_name = agg.operation_name
WHERE r.run_id = %d AND agg.avg_seconds > 0
GROUP BY i.engine, i.operation_name
ORDER BY i.engine, i.operation_name
      """ % (
        ",".join(last_revs)
      , run_id
      )
...
def printResults(results, report_text):
  ...
  for result in results:
    if result[1] != "TOTALS":
      report_text= report_text + "%-7s %-33s %8s %12s %10s %8s %7s %7s %7s\n" % tuple(result)
  return report_text
...
""" % (string)
  ...
  report_text= report_text + "%-7s %-36s %-6s %-17s %-8s %-7s %-7s %-7s %-7s" % ("Engine","Test","Time","% Diff from Avg","Diff","Min","Max","Avg","STD")
...
def getSqlbenchReport(working_dir, bench_config_name, run_id, run_date, server_name, bzr_branch, bzr_revision, storage_engine):
